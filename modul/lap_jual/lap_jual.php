...
$(document).ready(function() {
	$("#data").load('modul/lap_jual/tampil_data.php');
	...
	$("#cetak").click(function() {
		var kode1	= $("#kode1").val();
		var kode2	= $("#kode2").val();
		window.location.href='modul/laporan/lap_beli.php?kode1='+kode1+'&kode2='+kode2;
		/*
	   	$.ajax({
			type	: "GET",
			url		: "modul/laporan/lap_jual.php",
			data	: "kode1="+kode1+"&kode2="+kode2,
			success	: function(data){
				//$("#data").html(data);
...
<?php
echo "<div id='form' align='center'><h2>LAPORAN DATA PENJUALAN <h3></div>";
echo "<div id='filter' align='center'>
	<fieldset>
		<legend>Filter Data </legend>
		<table width='100%'>
			<tr>
				<td width='10%'>Kode Jual</td>
				<td width='2%'>:</td>
				<td ><select nama='kode1' id='kode1'>
				<option value=''>-Pilih Kode-</option>";
				$query	= "SELECT * FROM jual ORDER BY kode_barang";
				$sql	= mysql_query($query);
				while($r_data=mysql_fetch_array($sql)){
					echo "<option value='".$r_data['kode_beli']."'>".$r_data['kode_barang']." - ".$r_data['nama_barang']."</option>";
				}
				echo "</select>
				</td>
